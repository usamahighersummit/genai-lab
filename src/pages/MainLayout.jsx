...
import { Mic, MicOff, Play, Terminal, ChevronRight, Code2, Lightbulb, Info, TestTube, CheckCircle2 } from "lucide-react";
...
import LessonView from "../Components/MainLayout/LessonView";
import { programmingLessons } from "../Constants/lessons";
...
function MainLayout() {
  ...
  const currentLesson = programmingLessons[currentLessonIndex];
  ...
  function handleEditorDidMount(editor) {
    ...
    editor.setValue(currentLesson.practice.solutions[selectedLanguage].starterCode);
  }
  ...
  function handleNextLesson() {
    if (currentLessonIndex < programmingLessons.length - 1) {
      setCurrentLessonIndex(currentLessonIndex + 1);
      setIsLessonMode(true); // Return to lesson mode
      setOutput(""); // Clear output
      setTranscription(false);
      if (editorRef.current) {
        editorRef.current.setValue(programmingLessons[currentLessonIndex + 1].practice.solutions[selectedLanguage].starterCode);
      }
    }
  }
    ...
    try {
      ...
      if (data.status === "success" && data.transcription) {
        ...
        const commandResponse = await fetch("http://localhost:8000/process-voice-command", {
          ...
          body: JSON.stringify({
            command: userQuery,
            currentQuestion: currentLesson.practice,
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-900">
      ...
      <div className="flex-1 flex overflow-hidden">
        {isLessonMode ? (
          <LessonView lesson={programmingLessons[currentLessonIndex]} onStartPractice={handleStartPractice} />
        ) : (
          <>
            {/* Question Panel */}
              ...
              <div className="p-6">
                <div className="flex justify-between items-center mb-6">
                  <span className="px-3 py-1 bg-blue-500/10 text-blue-400 rounded-full text-sm border border-blue-500/20">
                    Lesson {currentLessonIndex + 1} of {programmingLessons.length}
                  </span>
                  <span
                    className={`px-3 py-1 rounded-full text-sm ${
                      currentLesson.practice.difficulty === "Easy"
                        ? "bg-green-500/10 text-green-400 border border-green-500/20"
                        : currentLesson.practice.difficulty === "Medium"
                        ? "bg-yellow-500/10 text-yellow-400 border border-yellow-500/20"
                        : "bg-red-500/10 text-red-400 border border-red-500/20"
                    }`}
                  >
                    {currentLesson.practice.difficulty}
                  </span>
                </div>

                <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                  <TestTube className="w-5 h-5 text-blue-400" />
                  {currentLesson.practice.title}
                </h2>

                <div className="prose max-w-none text-gray-300">
                  <p className="mb-6">{currentLesson.practice.description}</p>
                  ...
                  <div className="mb-6">
                    ...
                    {currentLesson.practice.examples.map((example, index) => (
                      <div key={index} className="bg-gray-900/50 p-4 rounded-lg mb-3 border border-gray-700">
                        ...
                        <div>
                          ...
                          <code className="ml-2 text-gray-300 font-mono text-sm" dangerouslySetInnerHTML={{ __html: example.output }}></code>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="mb-6">
                    ...
                    <ul className="space-y-2">
                      {currentLesson.practice.hints.map((hint, index) => (
                ...
                <div className="flex justify-end mt-6 pt-6 border-t border-gray-700">
                  <button
                    ...
                    disabled={currentLessonIndex === programmingLessons.length - 1}
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${
                      currentLessonIndex === programmingLessons.length - 1
